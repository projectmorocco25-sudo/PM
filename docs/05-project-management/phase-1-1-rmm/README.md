...
**🟢 Supabase cloud-only (never local):** All Phase 1.1 tasks use the **hosted** Supabase project only. Never use local Supabase, `supabase start`, Docker, or a local database. See [phase-1-1-rmm.md § Supabase cloud-only](../phase-1-1-rmm.md#-supabase-cloud-only-no-local).
...
└── tasks/
    ...
    ├── infrastructure/    # Infrastructure setup tasks (Supabase, Edge Functions, etc.)
    ├── migrations/        # Database migration tasks
    ├── testing/          # Integration and unit test tasks
    ├── documentation/    # Documentation and review tasks
    └── validation/       # Seed data and checkpoint validation tasks
```
...
2. **By Category:** Browse `tasks/frontend/`, `tasks/backend/`, `tasks/infrastructure/`, `tasks/migrations/`, `tasks/testing/`, `tasks/documentation/`, or `tasks/validation/`
