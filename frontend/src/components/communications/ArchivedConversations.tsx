...
import { useArchivedConversations } from "@/hooks/useCommunications";

export function ArchivedConversations() {
  const [search, setSearch] = useState("");
  const { data, isLoading } = useArchivedConversations(search, 25);
  const items = useMemo(() => data ?? [], [data]);

  return (
    <div>
      <div className="mb-4 text-xl font-semibold text-zinc-900">Archived</div>
      ...
      <div className="rounded-lg border border-zinc-200 bg-white">
        {isLoading ? (
          ...
          <div className="p-6 text-sm text-zinc-600">No archived conversations.</div>
        ) : (
              ...
              <li key={c.id}>
                <Link href={`/communications/inbox/${c.id}`} className="block px-4 py-4 hover:bg-zinc-50">
                  ...
                  <div className="mt-1 text-xs text-zinc-500">
                    Archived at {c.archived_at ? new Date(c.archived_at).toLocaleString() : "—"}
                  </div>
                </Link>
              </li>
            ))}
