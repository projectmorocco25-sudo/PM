-- Migration: fix_rmm_get_recent_activity_group_by
-- Description: Fix GROUP BY error in rmm_get_recent_activity - ORDER BY must be inside jsonb_agg or applied to subquery
-- Date: 2026-01-25
-- Task: Phase 1 - RMM Overview Page Fix
-- Author: Sami (Implementation Compliance Specialist)
-- Issue: "column 'activity.timestamp' must appear in the GROUP BY clause" - ORDER BY after jsonb_agg is invalid

BEGIN;

-- Drop and recreate the function with fixed ORDER BY placement
...
BEGIN
    ...
    -- Order and limit in subquery, then aggregate
    SELECT jsonb_agg(
        jsonb_build_object(
            'type', activity.type,
    ...
    FROM (
        ...
        FROM (
            ...
            -- Submission activities (filter by company through entity relationships)
            SELECT 
                CASE 
